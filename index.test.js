...
describe('[Exercise 2] trimPropertiesMutation', () => {
    test('[2] returns an object with the properties trimmed', () => {
      const input = { foo: '  foo ', bar: 'bar ', baz: ' baz' }
      const expected = { foo: 'foo', bar: 'bar', baz: 'baz' }
      const actual = utils.trimProperties(input)
      expect(actual).toEqual(expected)
...
describe('[Exercise 7] isEvenNumberAsync', () => {
      ...
      test('[21] resjects an error with the message "number must be a number" if passed by a non-number type', async () => {
        try {
      ...
      test('[22] resjects an error with the message "number must be a number" if passed NaN', async () => {
        try {
